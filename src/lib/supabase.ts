...
// Helper function to check and process rank-ups based on current XP
export const checkAndProcessRankUps = async (userId: string) => {
  try {
    ...
    // Only update if there were rank-ups
    if (rankedUp) {
      const updateData: any = { 
        current_xp: currentXp,
        current_rank: currentRankId,
        raid_points: (profile.raid_points || 0) + totalPointsEarned
      };
      ...
      if (updateError) {
        console.error('Error updating profile during rank-up:', updateError);
        throw updateError;
      }
      
      console.log(`Processed rank-ups! New rank: ${currentRankId}, XP: ${currentXp}, Earned ${totalPointsEarned} points.`);
    }
    
    return { success: true, rankedUp, pointsEarned: totalPointsEarned, newRank: currentRankId, newXp: currentXp };
  } catch (error) {
    console.error('Error in checkAndProcessRankUps:', error);
    throw error;
  }
};
